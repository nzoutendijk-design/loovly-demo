...
## Ink (light / dark) on themes

- Built the client's own rule from loovly.suncraft.studio/theme: each theme's ground luminance (WCAG, lower two-thirds of the mobile crop) decides the ink; above **0.1836** (where white text drops under AA 4.5:1) the whole screen flips to dark ink `22 24 29`, driven by one `--ink-rgb` token. 84 of 115 themes take dark ink; values are in `src/themes.json` (`ground`, `luma`, `inkDark`), regenerated by `scripts/theme-luma.py`.
- [ ] A per-theme `ink: "light" | "dark"` override exists for photos and illustrations where the average misleads; none set yet. **Needs the eyeball pass** over the photo (23) and illustration (44) themes.
- [ ] Sheets keep light ink because their backdrop is near-black on every theme; the card caption keeps the user's colour. **decide** whether that matches the intent.
- [ ] The client tool's stronger "solid dark buttons" mode (`ink-dark`) is switched off there and not built here.
